...
@section('title', __('New Note') . ' - Pro Contact')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('New Note') }}</h1>
            <p class="text-gray-600 mt-2">{{ __('Create a new personal note') }}</p>
        </div>
            ...
            <form method="POST" action="{{ route('notes.store') }}">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <!-- Title -->
                        <div>
                            <label for="titre" class="block text-sm font-medium text-gray-700 mb-2">{{ __('Title') }} *</label>
                    ...
                    <div class="space-y-6">
                        <!-- Priority -->
                        <div>
                            <label for="priorite" class="block text-sm font-medium text-gray-700 mb-2">{{ __('Priority') }} *</label>
                            <select id="priorite" name="priorite" required
                                ...
                                <option value="Basse" {{ old('priorite') == 'Basse' ? 'selected' : '' }}>{{ __('Low') }}</option>
                                <option value="Normale" {{ old('priorite', 'Normale') == 'Normale' ? 'selected' : '' }}>{{ __('Normal') }}</option>
                                <option value="Haute" {{ old('priorite') == 'Haute' ? 'selected' : '' }}>{{ __('High') }}</option>
                                <option value="Urgente" {{ old('priorite') == 'Urgente' ? 'selected' : '' }}>{{ __('Urgent') }}</option>
                            </select>
                            @error('priorite')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <!-- Associated Contact -->
                        <div>
                            <label for="contact_id" class="block text-sm font-medium text-gray-700 mb-2">{{ __('Associated contact') }}</label>
                        ...
                        <!-- Associated Activity -->
                        <div>
                            <label for="activite_id" class="block text-sm font-medium text-gray-700 mb-2">{{ __('Associated activity') }}</label>
                            <select id="activite_id" name="activite_id"
                                ...
                                <option value="">{{ __('No activity') }}</option>
                        ...
                        <div>
                            <label for="rendez_vous_id" class="block text-sm font-medium text-gray-700 mb-2">{{ __('Associated appointment') }}</label>
                            <select id="rendez_vous_id" name="rendez_vous_id"
                                ...
                                <option value="">{{ __('No appointment') }}</option>
                        ...
                        <!-- Quick Templates -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-3">{{ __('Quick templates') }}</label>
                            <div class="space-y-2">
                        ...
                        @if($contact || $activite || $rendezVous)
                            <div class="p-4 bg-indigo-50 rounded-lg">
                                <h4 class="text-sm font-medium text-indigo-900 mb-2">{{ __('Pre-selected items:') }}</h4>
                                @if($contact)
                                    <p class="text-sm text-indigo-700">{{ __('Contact:') }} {{ $contact->prenom }} {{ $contact->nom }}</p>
                                @endif
                                @if($activite)
                                    <p class="text-sm text-indigo-700">{{ __('Activity:') }} {{ $activite->nom }}</p>
                                @endif
                                @if($rendezVous)
                                    <p class="text-sm text-indigo-700">{{ __('Appointment:') }} {{ $rendezVous->titre }}</p>
                                @endif
                            </div>
                        @endif
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    templateButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (confirm('{{ __('Use this template? This will replace the current content.') }}')) {
